...
- **`tools/tes3mp-server-update`** — sync plugins and server scripts from your local machine to the server
- **`tools/tes3mp-client-update`** — download and install the latest plugins from the server

## Repository layout

```
setup/       — server setup (Docker, install/update scripts)
tools/       — client and server synchronisation utilities
